...
function createSphere(gl, radius)
{
	...
	//función para llenar nuestra variable verts con las coordenadas del circulo unitario
	var verts = [0.0, 0.0, 0.0]; //inicializar con el pimer vértice es el centro
	var numPoints = 50; // cuantas coordenadas queremos para nuestro círculo
	var multiplier = 0.5; //estaba demasiado grande, lo vamos a multiplicar por un decimal para reducir el radio
	for (var a = 0; a < numPoints; a++){
		verts.push(multiplier*Math.cos(2*Math.PI * a/numPoints)); //llenar coordenada x
    	verts.push(multiplier*Math.sin(2*Math.PI * a/numPoints)); //llenar coordenada y
		verts.push(0.0); //llenar coordenada z
	}
